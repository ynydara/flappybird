...
def main():
    ...
    while running:
        ...
        if p.time.get_ticks() % 120 == 0:
            pipe_height = generate_pipe_height(lastPipeHeight)
            a = random.randint(0, 99)  
            if a % 3 == 0:
                a += 100
            a = random.randint(0,100)
            upper_pipe = Pipe(screenWidth+a, pipe_height, True)
            lower_pipe = Pipe(screenWidth+a, pipe_height + 200, False)
